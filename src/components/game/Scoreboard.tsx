 ...
 * Scoreboard
 *
 * Zeigt alle Spieler/Teams mit ihrem aktuellen Score, sortiert nach Punkten.
 * Wird als seitlicher Overlay oder Bottom-Sheet eingeblendet.
 */
'use client';

import { motion, AnimatePresence } from 'framer-motion';
import type { Player, Team, TeamMode } from '@/types/player';
...
export function Scoreboard({ players, teams = [], teamMode = 'individual', winCondition, isOpen, onClose }: ScoreboardProps) {
  const isTeamMode = teamMode !== 'individual' && teams.length > 0;

  // Spieler oder Teams nach Score sortieren (höchster zuerst)
            ...
            <ol className="flex-1 overflow-y-auto p-4 space-y-2">
              {isTeamMode ? (
                ...
                sortedTeams.map((team, index) => {
                  const progressPct = Math.min((team.score / winCondition) * 100, 100);
                  ...
                  return (
                    <li key={team.id}>
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-sm font-black w-5 text-center shrink-0 opacity-60">
                          {index + 1}.
                        </span>
                        <div
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1">
                            <span className="font-bold text-sm truncate" style={{ color: team.color }}>
                              {isLeading && '👑 '}{team.name}
                            </span>
                            <span className="font-black text-sm shrink-0">{team.score}</span>
                          </div>
                          <div className="h-1.5 rounded-full mt-1 overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
                            <motion.div
                          ...
                          {memberNames && (
                            <p className="text-[10px] opacity-40 mt-1 truncate">{memberNames}</p>
                          )}
                        </div>
                      </div>
                    </li>
                      ...
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1">
                            ...
                            <span className="font-black text-sm shrink-0">{player.score}</span>
                          </div>
                          <div
